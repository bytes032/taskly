...
class ConvertButtonWidget extends WidgetType {
	...
	constructor(plugin: TasklyPlugin, lineNumber: number) {
		super();
		this.plugin = plugin;
	...
	toDOM(view: EditorView): HTMLElement {
		...
		container.className = "taskly-plugin";

		const button = container.createEl("button", {
			cls: "instant-convert-button",
			attr: { "aria-label": "Convert to Taskly note" },
		});
		setTooltip(button, "Convert to Taskly note", { placement: "top" });
...
export function createInstantConvertField(plugin: TasklyPlugin) {
...
function buildConvertButtonDecorations(
	...
	// Don't show convert buttons in files that are already Taskly tasks
	const activeFile = plugin.app.workspace.getActiveFile();
	if (activeFile && plugin.cacheManager) {
		const metadata = plugin.app.metadataCache.getFileCache(activeFile);
		if (metadata?.frontmatter && plugin.cacheManager.isTaskFile(metadata.frontmatter)) {
			return builder.finish();
		}
	}
...
export function createInstantConvertButtons(plugin: TasklyPlugin): Extension {
