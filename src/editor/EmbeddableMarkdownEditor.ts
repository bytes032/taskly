...
export class EmbeddableMarkdownEditor extends getEditorBase() {
	...
	constructor(app: App, container: HTMLElement, options: Partial<MarkdownEditorProps> = {}) {
		...
		// Override Mod+Enter to prevent default workspace behavior and submit when handled
	 ...
	 */
	focus(): void {
		if (this.editor?.cm?.dom?.isConnected) {
			this.editor.cm.focus();
		}
	}

	/**
	 * Get the scroll DOM element (cm-scroller)
	 */
	get scrollDOM(): HTMLElement {
		return this.editor.cm.scrollDOM;
	}
