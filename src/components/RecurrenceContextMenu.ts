...
export interface RecurrenceOption {
	...
	value: string;
	icon?: string;
	anchor?: 'due' | 'completion'; // Determines if recurrence is from due date or completion date
}

export interface RecurrenceContextMenuOptions {
	currentValue?: string;
	currentAnchor?: 'due' | 'completion';
	onSelect: (value: string | null, anchor?: 'due' | 'completion') => void;
	app: App;
	plugin: TasklyPlugin;
}
...
export class RecurrenceContextMenu {
	...
	private buildMenu(): void {
		...
		recurrenceOptions.forEach((option) => {
			...
			this.menu.addItem((item) => {
				...
				item.onClick(async () => {
					const anchorValue = option.anchor || 'due';
	...
	private getRecurrenceOptions(): RecurrenceOption[] {
		...
		options.push({
			label: formatString("Weekly on {day}",  { day: dayName }),
		...
		options.push({
			label: formatString("Every 2 weeks on {day}",  {
				day: dayName,
			}),
		...
		options.push({
			label: formatString("Every 3 months on the {ordinal}",  {
				ordinal: this.getOrdinal(currentDate),
			}),
		...
		options.push({
			label: formatString("Yearly on {month} {ordinal}",  {
		...
		options.push({
			label: "Every 3 days (after completion)",
		...
		options.push({
			label: "Monthly (after completion)",
	...
	private showCustomRecurrenceModal(): void {
		new CustomRecurrenceModal(
			...
			this.options.currentAnchor || 'due',
			(result, anchor) => {
				if (result) {
...
class CustomRecurrenceModal extends Modal {
	...
	private onSubmit: (result: string | null, anchor?: 'due' | 'completion') => void;
	...
	private dtstart = "";
	private dtstartTime = "";
	private recurrenceAnchor: 'due' | 'completion' = 'due'; // Recurrence anchor

	constructor(app: App, currentValue: string, currentAnchor: 'due' | 'completion', onSubmit: (result: string | null, anchor?: 'due' | 'completion') => void) {
		super(app);
