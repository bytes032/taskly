...
export interface TaskAction {
	...
	description: string;
	icon: string;
	category: "status" | "dates" | "other";
	keywords: string[];
	isApplicable: (task: TaskInfo, plugin: TasklyPlugin, targetDate: Date) => boolean;
	execute: (task: TaskInfo, plugin: TasklyPlugin, targetDate: Date) => Promise<void>;
}
...
export class TaskActionPaletteModal extends FuzzySuggestModal<TaskAction> {
	...
	constructor(app: App, task: TaskInfo, plugin: TasklyPlugin, targetDate: Date) {
		super(app);
		this.task = task;
		this.plugin = plugin;
