...
export class TaskSelectorWithCreateModal extends SuggestModal<TaskInfo> {
	...
	constructor(
		...
		// Set placeholder
		this.setPlaceholder(
			options.placeholder || "Search tasks or type to create new..."
		);
		...
		this.titleEl.setText(
			options.title || "Create or open task"
		);
		...
		// Add taskly-plugin class so task card styles apply
		this.containerEl.addClass("taskly-plugin");
	}
	...
	private async createNewTask(): Promise<void> {
		...
		try {
			...
			if (!parsed.title || parsed.title === "Untitled Task") {
				new Notice("Could not parse a valid task title");
				return;
			}
			...
			new Notice(
				formatString("Task \"{title}\" created successfully",  { title: result.taskInfo.title })
			);
 ...
 *
 * @param plugin - The Taskly plugin instance
