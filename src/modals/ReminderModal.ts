...
export class ReminderModal extends Modal {
	...
	private relativeAnchor: "due" = "due";
	...
	onOpen() {
		...
		contentEl.empty();
		contentEl.addClass("taskly-plugin");
		contentEl.addClass("taskly-reminder-modal");
	...
	private renderQuickActions(section: HTMLElement): void {
		// Only show quick actions if task has a due date
		const hasDates = this.task.due;
		if (!hasDates) return;
	...
	private renderAddReminderForm(container: HTMLElement): void {
		...
		new Setting(relativeContainer).setName("Relative to").addDropdown((dropdown) => {
			...
			dropdown.onChange((value) => {
				this.relativeAnchor = value as "due";
			});
		});
	...
	private createReminder(
		...
		if (type === "relative") {
			...
			if (!anchorDate) {
				new Notice("Cannot create reminder: Task has no due date");
				return null;
			}
