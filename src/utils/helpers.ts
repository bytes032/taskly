import { normalizePath, TFile, Vault, App, parseYaml } from "obsidian";
...
export function getNextUncompletedOccurrence(task: TaskInfo): Date | null {
	...
	const anchor = task.recurrence_anchor || 'due'; // Default to due
	...
	if (anchor === 'completion') {
		...
		// For due-based recurrence, DTSTART is fixed
		// We need to generate occurrences and filter out completed ones
		return getNextDueBasedOccurrence(task);
	}
}

/**
 * Gets next occurrence for due-based (fixed) recurrence
 */
function getNextDueBasedOccurrence(task: TaskInfo): Date | null {
	if (!task.recurrence) {
		return null;
	}
	...
	try {
		...
		console.error("Error calculating next due-based occurrence:", error, {
			task: task.title,
		});
		return null;
	}
...
/**
 * Updates the due date of a recurring task to its next uncompleted occurrence
 * Returns the updated due date or null if no next occurrence
 * @param task Task info object
 */
export function updateToNextDueOccurrence(
	task: TaskInfo
): { due: string | null } {
	...
	if (nextOccurrence) {
		// Preserve time component if original due date had time
		if (task.due && task.due.includes("T")) {
			const timePart = task.due.split("T")[1];
			nextDueStr = `${formatDateForStorage(nextOccurrence)}T${timePart}`;
		} else {
			nextDueStr = formatDateForStorage(nextOccurrence);
		}
	}
 ...
 * as the recurrence interpretation (due date first, then dateCreated)
 ...
 * Uses fallback logic for the date (due first, then dateCreated) but applies the user-dragged time
 * Follows the UTC Anchor principle for consistent date handling
 */
