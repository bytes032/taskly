 ...
 *
 * Special transformations (for computed properties): none for core fields
 */
export class PropertyMappingService {
	 ...
	 * @returns TaskCard property ID (e.g., "complete_instances", "title")
	 *
	 * @example
	...
	basesToTaskCardProperty(basesPropertyId: string): TaskCardPropertyId {
		...
		if (this.fieldMapper) {
			...
			// Also check if it's a FieldMapping key (e.g., "status" when mapped to "done")
			// This handles existing .base files that reference old field names
			const mapping = this.fieldMapper.getMapping();
			if (basesPropertyId in mapping) {
				return this.applySpecialTransformations(
					mapping[basesPropertyId as keyof typeof mapping]
				);
			}
		}
		...
		if (basesPropertyId.startsWith("note.")) {
			...
			// Check if stripped name is a FieldMapping key (e.g., "status" when mapped to "done")
			if (this.fieldMapper) {
				const mapping = this.fieldMapper.getMapping();
				if (stripped in mapping) {
					return this.applySpecialTransformations(
						mapping[stripped as keyof typeof mapping]
					);
				}
			}
		...
		if (basesPropertyId.startsWith("task.")) {
			...
			// Check if stripped name is a FieldMapping key
			if (this.fieldMapper) {
				const mapping = this.fieldMapper.getMapping();
				if (stripped in mapping) {
					return this.applySpecialTransformations(
	 ...
	 * @returns TaskInfo property name (e.g., "status", "due")
	 *
	 * @example
