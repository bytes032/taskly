 ...
 *
 * Transformations:
 * - All properties pass through unchanged
 */
...
/**
 * Identify Taskly from Bases data by converting all items to TaskInfo
 */
export async function identifyTasklyFromBasesData(
	dataItems: BasesDataItem[],
	plugin?: TasklyPlugin,
	toTaskInfo?: (item: BasesDataItem, plugin?: TasklyPlugin) => TaskInfo | null
): Promise<TaskInfo[]> {
	...
	for (const item of dataItems) {
		...
		try {
			...
			if (taskInfo) taskItems.push(taskInfo);
		} catch (error) {
			console.warn("[Taskly][BasesPOC] Error converting Bases item to TaskInfo:", error);
		}
	}
	return taskItems;
}
...
/**
 * Render Taskly using TaskCard component into a container
 */
...
export function getBasesVisibleProperties(basesContainer: any): BasesSelectedProperty[] {
	try {
		...
		console.log("[Taskly][Bases] getBasesVisibleProperties - controller:", !!controller, "query:", !!query);

		if (!controller) {
			console.log("[Taskly][Bases] getBasesVisibleProperties - no controller, returning empty");
			return [];
		}
		...
		if (config && typeof config.getOrder === "function") {
			try {
				...
				console.log("[Taskly][Bases] getBasesVisibleProperties - got order from config.getOrder():", order);
			} catch (e) {
				console.log("[Taskly][Bases] getBasesVisibleProperties - config.getOrder() failed:", e);
			}
		}
		...
		if (!order || !Array.isArray(order) || order.length === 0) {
			...
			try {
				...
				if (order && Array.isArray(order) && order.length > 0) {
					console.log("[Taskly][Bases] getBasesVisibleProperties - got order from internal API:", order);
				}
			} catch (_) {
		...
		if (!order || !Array.isArray(order) || order.length === 0) {
			console.log("[Taskly][Bases] getBasesVisibleProperties - no order found, returning empty. order:", order);
			return [];
		}
		...
		console.log("[Taskly][Bases] getBasesVisibleProperties failed:", e);
		return [];
	}
}

export async function renderTasklyInBasesView(
	...
	console.log("[Taskly][Bases] renderTasklyInBasesView ENTRY - tasks:", taskItems.length, "basesContainer:", !!basesContainer, "precomputed props:", precomputedVisibleProperties?.length);
	...
	taskListEl.className = "tn-bases-taskly-list";
	...
	if (!visibleProperties && basesContainer) {
		console.log("[Taskly][Bases] basesContainer type:", typeof basesContainer, "keys:", basesContainer ? Object.keys(basesContainer).slice(0, 10) : []);
		const basesVisibleProperties = getBasesVisibleProperties(basesContainer);
		console.log("[Taskly][Bases] getBasesVisibleProperties returned:", basesVisibleProperties.length, "properties");
		...
		if (basesVisibleProperties.length > 0) {
			...
			console.log("[Taskly][Bases] Raw property IDs from Bases:", visibleProperties);

			// Map Bases property IDs to TaskCard-compatible property names
			visibleProperties = visibleProperties.map((propId) => {
				const mapped = mapBasesPropertyToTaskCardProperty(propId, plugin);
				if (propId !== mapped) {
					console.log(`[Taskly][Bases] Mapped ${propId} → ${mapped}`);
				}
				return mapped;
			});
			console.log("[Taskly][Bases] Final visible properties:", visibleProperties);
		}
	}
	...
	if (!visibleProperties || visibleProperties.length === 0) {
		...
		console.log("[Taskly][Bases] Using default properties (filtered):", visibleProperties);
	}
	...
	for (const taskInfo of taskItems) {
		try {
			...
			console.warn("[Taskly][BasesPOC] Error creating task card:", error);
		}
	}
}

/**
 * Render grouped Taskly in Bases list view
...
export async function renderGroupedTasksInBasesView(
	...
	if (basesVisibleProperties.length > 0) {
		visibleProperties = basesVisibleProperties.map((p) => p.id);
		console.log("[Taskly][Bases][Grouped] Raw property IDs from Bases:", visibleProperties);

		// Map Bases property IDs to TaskCard-compatible property names
		visibleProperties = visibleProperties.map((propId) => {
			const mapped = mapBasesPropertyToTaskCardProperty(propId, plugin);
			if (propId !== mapped) {
				console.log(`[Taskly][Bases][Grouped] Mapped ${propId} → ${mapped}`);
			}
			return mapped;
		});
		console.log("[Taskly][Bases][Grouped] Final visible properties:", visibleProperties);
	}
	...
	if (!Array.isArray(groupedData) || groupedData.length === 0) {
		...
		await renderTasklyInBasesView(container, taskItems, plugin, viewContext, taskElementsMap, visibleProperties);
		return;
	}
	...
	listWrapper.className = "tn-bases-taskly-list";
	...
	for (const group of groupedData) {
		...
		if (groupKey instanceof Date) {
			// Format dates as YYYY-MM-DD for consistency with Taskly date handling
		...
		for (const task of groupTasks) {
			try {
				...
				console.warn("[Taskly][Bases] Error creating task card:", error);
			}
		}
	}
}
