...
export function getBasesAPI(app: App): BasesAPI | null {
	try {
		...
		if (!internalPlugins) {
			console.debug("[Taskly][Bases] Internal plugins manager not available");
			return null;
		}

		const basesPlugin = internalPlugins.getEnabledPluginById?.("bases");
		if (!basesPlugin) {
			console.debug("[Taskly][Bases] Bases plugin not found or not enabled");
			return null;
		}
		...
		if (!basesPlugin.registrations || typeof basesPlugin.registrations !== "object") {
			console.warn(
				"[Taskly][Bases] Bases plugin found but registrations API not available"
			);
			return null;
		}
		...
		console.warn("[Taskly][Bases] Error accessing Bases plugin API:", error);
		return null;
	}
}
...
export function registerBasesView(
	...
	if (typeof (plugin as any).registerBasesView === "function") {
		try {
			...
			if (success) {
				console.debug(
					`[Taskly][Bases] Successfully registered view via public API: ${viewId}`
				);
				return true;
			}
			console.debug(
				`[Taskly][Bases] Public API returned false (Bases may be disabled)`
			);
			return false;
		} catch (error: any) {
			...
			if (error?.message?.includes("already exists")) {
				console.debug(
					`[Taskly][Bases] View ${viewId} already registered via public API`
				);
				return true;
			}
			console.warn(
				`[Taskly][Bases] Public API registration failed for ${viewId}:`,
				error
			);
			return false;
		}
	...
	console.warn("[Taskly][Bases] Cannot register view: Bases public API not available (requires Obsidian 1.10.0+)");
	return false;
}
...
export function unregisterBasesView(plugin: Plugin, viewId: string): boolean {
	...
	try {
		...
		console.error(`[Taskly][Bases] Error unregistering view ${viewId}:`, error);
		return false;
	}
}
