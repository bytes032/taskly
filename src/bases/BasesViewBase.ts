...
/**
 * Abstract base class for all Taskly Bases views.
...
export abstract class BasesViewBase extends Component {
	 ...
	 */
	onload(): void {
		this.onloadTime = performance.now();
		const sinceConstruct = this.onloadTime - this.constructTime;
		console.log(`[Taskly][Perf] ${this.type} onload() called ${sinceConstruct.toFixed(1)}ms after constructor`);
	 ...
	 */
	onDataUpdated(): void {
		// Log time-to-first-data on first call
		const isFirstDataUpdate = !this.hasReceivedFirstData;
		if (isFirstDataUpdate) {
			this.hasReceivedFirstData = true;
			this.firstDataTime = performance.now();
			const sinceConstruct = this.firstDataTime - this.constructTime;
			const sinceOnload = this.firstDataTime - this.onloadTime;
			console.log(`[Taskly][Perf] ${this.type} FIRST onDataUpdated() - time-to-first-data: ${sinceConstruct.toFixed(1)}ms since constructor, ${sinceOnload.toFixed(1)}ms since onload`);
		}
		...
		// First data update: render immediately for fastest initial paint
		if (isFirstDataUpdate) {
			try {
				this.updateRelevantPathsCache();
				this.render();
			} catch (error) {
				console.error(`[Taskly][${this.type}] Render error:`, error);
		...
		}, this.getDataUpdateDebounceMs());
	}

	protected getDataUpdateDebounceMs(): number {
		return 500;
	}
	...
	setEphemeralState(state: any): void {
		...
		try {
			...
			console.debug("[Taskly][Bases] Failed to restore ephemeral state:", e);
		}
	}
	...
	focus(): void {
		try {
			...
			console.debug("[Taskly][Bases] Failed to focus view:", e);
		}
	}
	...
	protected setupContainer(): void {
		...
		root.className = `tn-bases-integration taskly-plugin taskly-container tn-${this.type}`;
	...
	/**
	 * Setup custom "New Task" button that opens Taskly creation modal.
	...
	private cleanupNewTaskButton(): void {
		...
		parentEl?.classList.remove("taskly-view-active");
	}
	...
	private injectNewTaskButton(): void {
		...
		const basesViewEl = this.containerEl.closest(".bases-view");
		if (!basesViewEl) {
			console.debug("[Taskly][Bases] No .bases-view found");
			return;
		}
		...
		const parentEl = basesViewEl.parentElement;
		if (!parentEl) {
			console.debug("[Taskly][Bases] No parent element found");
			return;
		}

		// Mark parent as having an active Taskly view (controls visibility via CSS)
		parentEl.classList.add("taskly-view-active");

		const toolbarEl = parentEl.querySelector(".bases-toolbar");
		if (!toolbarEl) {
			console.debug("[Taskly][Bases] No .bases-toolbar found in parent");
			return;
		}
		...
		console.debug("[Taskly][Bases] Injected New Task button into toolbar");
	}
	...
	protected setupTaskUpdateListener(): void {
		...
		this.taskUpdateListener = this.plugin.emitter.on(EVENT_TASK_UPDATED, async (eventData: any) => {
			try {
				...
				const taskPath = updatedTask?.path || eventData?.path;
				if (!taskPath) return;
				...
				const isRelevant = this.relevantPathsCache.has(taskPath);
				...
				console.error("[Taskly][Bases] Error in task update handler:", error);
				this.debouncedRefresh();
			}
		});
	...
	/**
	 * Override Bases "New" button to open Taskly creation modal instead of default file creation.
	 ...
	 *
	 * @param baseFileName - Suggested filename from Bases (typically unused in Taskly)
	...
	async createFileForView(
		...
		if (frontmatterProcessor) {
			...
			// Map core Taskly properties from frontmatter
	 ...
	 * Subclasses implement view-specific rendering (list, table).
	 */
