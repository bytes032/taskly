...
export class AutoArchiveService {
	...
	async scheduleAutoArchive(task: TaskInfo, statusConfig: StatusConfig): Promise<void> {
		...
		const now = Date.now();
		const delayMinutes = Math.max(0, statusConfig.autoArchiveDelay || 0);
		const archiveAfter = now + delayMinutes * 60 * 1000; // Convert minutes to ms
		...
		if (delayMinutes === 0) {
			const processed = await this.archiveNow(task, statusConfig.value);
			if (processed) return;
		}
	...
	private async archiveNow(task: TaskInfo, statusValue: string): Promise<boolean> {
		...
		try {
			...
			console.error(`Failed to archive task ${task.path}:`, error);
			return false;
		}
	}
