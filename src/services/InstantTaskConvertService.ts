...
export class InstantTaskConvertService {
	...
	/**
	 * Batch convert all checkbox tasks in the current note to Taskly (optimized version)
	 */
	async batchConvertAllTasks(editor: Editor): Promise<void> {
		try {
			...
			new Notice(
				formatString("Converting {count} task{plural}...",  {
			...
			if (result.failures.length === 0) {
				...
				new Notice(
					formatString("✅ Successfully converted {count} task{plural} to Taskly!",  {
				...
				new Notice(
					formatString("Converted {successCount} task{successPlural}. {failureCount} failed.",  {
	...
	/**
	 * Instantly convert a checkbox task to a Taskly note without showing the modal
	...
	async instantConvertTask(editor: Editor, lineNumber: number): Promise<void> {
		try {
			...
			if (!validationResult.isValid) {
				new Notice("Invalid input parameters.");
				return;
			}
			...
			if (!taskLineInfo.isTaskLine) {
				// Line is not a checkbox task, but we can still convert it to a Taskly note
				...
				// Try NLP parsing first for better metadata extraction
				...
				if (taskLineInfo.error || !taskLineInfo.parsedData) {
					new Notice(
						formatString("Error parsing task: {error}",  {
			...
			if (!replaceResult.success) {
				new Notice("Failed to replace task line.");
			...
			if (actualFilename.startsWith("task-") && actualFilename !== expectedFilename) {
				new Notice(
					formatString("Task converted: \"{title}\" (filename shortened due to length)", 
						{ title: parsedData.title })
				);
			} else {
				new Notice(
					formatString("Task converted: {title}",  {
						title: parsedData.title,
					})
				);
			}
	...
	private validateTaskData(parsedData: ParsedTaskData): { isValid: boolean; error?: string } {
		...
		const dateFields = ["dueDate", "startDate", "createdDate", "doneDate"];
	...
	private async createTaskFile(parsedData: ParsedTaskData, details = ""): Promise<TFile> {
		...
		// Parse due date from task (if present)
		...
		if (this.plugin.settings.useDefaultsOnInstantConvert) {
			...
			// Apply status from parsed data or defaults
			status =
	...
	/**
	 * Replace the original task lines (including multi-line selection) with a link to the new Taskly note
	 */
	 ...
	 * TasksPlugin explicit metadata (emoji-based) takes precedence over NLP-inferred values.
	 * Arrays (tags) are combined and deduplicated.
	 */
