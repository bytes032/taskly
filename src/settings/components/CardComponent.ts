/**
 * ====================================================================
 * Taskly Card Component System
 * ====================================================================
 *
 * A comprehensive, reusable card UI system for Taskly settings.
 ...
 *   container.empty();
 *   const statusContainer = container.createDiv('taskly-statuses-container');
 *   plugin.settings.statuses.forEach(status => {
 *     const card = statusContainer.createDiv('taskly-status-card');
...
export function createCard(container: HTMLElement, config: CardConfig): HTMLElement {
	const card = container.createDiv("taskly-settings__card");
	...
	if (config.draggable) {
		card.addClass("taskly-settings__card--draggable");
	}

	if (config.collapsible) {
		card.addClass("taskly-settings__card--collapsible");
		if (config.defaultCollapsed) {
			card.addClass("taskly-settings__card--collapsed");
		}
	}

	// Create header
	const header = card.createDiv("taskly-settings__card-header");

	// Create drag handle if draggable - place it in the header
	let dragHandle: HTMLElement | null = null;
	if (config.draggable) {
		header.addClass("taskly-settings__card-header--with-drag-handle");
		dragHandle = header.createDiv("taskly-settings__card-drag-handle");
	...
	if (config.colorIndicator) {
		const colorIndicator = headerLeft.createDiv("taskly-settings__card-color-indicator");
	...
	// Header info
	const headerInfo = headerLeft.createDiv("taskly-settings__card-info");
	const primaryText = headerInfo.createSpan("taskly-settings__card-primary-text");
	primaryText.textContent = config.header.primaryText;

	if (config.header.secondaryText) {
		const secondaryText = headerInfo.createSpan("taskly-settings__card-secondary-text");
	...
	if (config.header.meta && config.header.meta.length > 0) {
		const headerMeta = headerRight.createDiv("taskly-settings__card-meta");
	...
	if (config.header.actions && config.header.actions.length > 0) {
		const headerActions =
			headerRight.querySelector(".taskly-settings__card-header-actions") ||
			headerRight.createDiv("taskly-settings__card-header-actions");
		config.header.actions.forEach((actionConfig) => {
			const button = headerActions.createEl("button", {
				cls: "taskly-settings__card-header-btn",
			});

			if (actionConfig.variant === "delete") {
				button.addClass("taskly-settings__card-header-btn--delete");
			}
	...
	if (config.collapsible) {
		...
		const toggleCollapse = () => {
			const isCurrentlyCollapsed = card.hasClass("taskly-settings__card--collapsed");

			if (isCurrentlyCollapsed) {
				// Expand
				card.removeClass("taskly-settings__card--collapsed");
		...
		header.addClass("taskly-settings__card-header--clickable");
		header.title = config.defaultCollapsed ? "Expand card" : "Collapse card";
		header.onclick = (e) => {
			// Don't trigger if clicking on action buttons
			if ((e.target as Element).closest(".taskly-settings__card-header-actions")) {
				return;
			}
			toggleCollapse();
		};
	...
	if (config.content && config.content.sections.length > 0) {
		const content = card.createDiv("taskly-settings__card-content");
		if (config.draggable) {
			content.addClass("taskly-settings__card-content--with-drag-handle");
		}

		config.content.sections.forEach((section) => {
			section.rows.forEach((row) => {
				const configRow = content.createDiv("taskly-settings__card-config-row");
				...
				const label = configRow.createSpan("taskly-settings__card-config-label");
	...
	if (config.actions && config.actions.buttons.length > 0) {
		const actions = card.createDiv("taskly-settings__card-actions");
		if (config.draggable) {
			actions.addClass("taskly-settings__card-actions--with-drag-handle");
		}

		config.actions.buttons.forEach((buttonConfig) => {
			const button = actions.createEl("button", {
				text: buttonConfig.text,
				cls: "taskly-settings__card-action-btn",
			});

			if (buttonConfig.variant) {
				button.addClass(`taskly-settings__card-action-btn--${buttonConfig.variant}`);
			}
...
export function createStatusBadge(
	...
	badge.addClass("taskly-settings__card-status-badge");
	badge.addClass(`taskly-settings__card-status-badge--${variant}`);
...
export function createCardSelect(
	...
	select.addClass("taskly-settings__card-input");
...
export function setupCardDragAndDrop(
	...
	const dragHandle = card.querySelector(".taskly-settings__card-drag-handle") as HTMLElement;
	...
	dragHandle.addEventListener("dragstart", (e) => {
		if (e.dataTransfer) {
			...
			card.addClass("taskly-settings__card--dragging");
		}
	});

	dragHandle.addEventListener("dragend", () => {
		card.removeClass("taskly-settings__card--dragging");
	});

	card.addEventListener("dragover", (e) => {
		e.preventDefault();
		const draggingCard = container.querySelector(
			".taskly-settings__card--dragging"
		) as HTMLElement;
		if (draggingCard && draggingCard !== card) {
			...
			card.removeClass(
				"taskly-settings__card--drag-over-top",
				"taskly-settings__card--drag-over-bottom"
			);

			if (e.clientY < midpoint) {
				card.addClass("taskly-settings__card--drag-over-top");
			} else {
				card.addClass("taskly-settings__card--drag-over-bottom");
			}
		}
	});
...
export function showCardEmptyState(
	...
	container.empty();
	const emptyState = container.createDiv("taskly-settings__empty-state");
	emptyState.createSpan("taskly-settings__empty-icon");
	emptyState.createSpan({
		text: message,
		cls: "taskly-settings__empty-text",
	});
